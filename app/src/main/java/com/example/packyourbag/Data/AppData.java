...
public class AppData extends Application {
    ...
    public AppData(RoomDB category, Context context) {
        this.database = database;
    ...
    public List<Items> getBabyNeedsData() {
        String[] data = {"Snapsuit", "Outfit", "Jumpsuit", "Baby Socks", "Baby Hat", "Baby Pyjamas",
                ...
                "Baby Care Cover", "Baby Shampoo", "Baby soap", "Baby Nail Scissors","Body Moisturizer", "Potty","diaper Rash Cream", "Serum Physiological",
    ...
    public List<List<Items>> getAllData() {
        ...
        listOfAllItems.add(getBeachSuppliesData()); listOfAllItems.add(getCarSuppliesData());
    ...
    private List<Items> deleteAndGetlListByCategory(String category, Boolean onlyDelete) {
        if (onlyDelete) {
            database.mainDao().deleteAllCategoryAndAddedBy(category, MyConstants.SYSTEM_SMALL);
        } else {
            database.mainDao().deleteAllCategory(category);
        }
        switch (category) {
